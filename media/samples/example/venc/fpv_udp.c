...
#define DEST_IP "192.168.3.98"  // <--- ВАШ IP
...
1. Запустить це на пристрої Rockchip з камерою.
  1.1. Для h264:  fpv_udp -w 1920 -h 1080 -a /oem/usr/share/iqfiles/ -I 0 -e h264cbr -b 3000 -o /tmp/
  1.2. Для h265:  fpv_udp -w 1920 -h 1080 -a /oem/usr/share/iqfiles/ -I 0 -e h265cbr -b 512 -o /tmp/

2. Налаштуйте go2rtc на прийом UDP-потоку:
  2.1. Створіть файл config.yaml з таким вмістом:
...
streams:
  ...
  luckfox_fpv5:
    - "exec:ffmpeg -fflags nobuffer -flags low_delay -f hevc -i udp://0.0.0.0:5000 -c:v copy -rtsp_transport tcp -f rtsp {output}"


Цей приклад оснований на sample_vi_vpss_osd_venc.
...
static void *venc_get_stream(void *pArgs) {
    ...
    // Ініціалізація сокета при старті потоку
    if (sockfd < 0) init_udp();
	...
	while (!quit) {
		...
		if (s32Ret == RK_SUCCESS) {
            ...
            // Отримуємо розмір всього кадру (SDK це гарантує)
            // Використовуємо ТОЧНО ТЕ САМЕ поле, що і в оригінальному fwrite
            uint32_t total_len = ctx->stFrame.pstPack->u32Len;
            uint8_t *ptr = (uint8_t *)pData;
            uint32_t sent = 0;
            uint32_t mtu = 1400; // Розмір шматка для UDP

            // Цикл розрізання і відправки (Fragmentation)
            while (sent < total_len) {
                uint32_t chunk = total_len - sent;
                if (chunk > mtu) chunk = mtu;

                if (sockfd >= 0) {
                    sendto(sockfd, ptr + sent, chunk, 0,
                           (const struct sockaddr *)&dest_addr, sizeof(dest_addr));
                }

                sent += chunk;
                // usleep(10); // Можна розкоментувати, якщо мережа перевантажена
            }

            // Логуємо раз на 100 кадрів, щоб не засмічувати консоль
            if (loopCount % 100 == 0) {
			    printf("Sending frame %d, size: %d\n", loopCount, total_len);
            }

			SAMPLE_COMM_VENC_ReleaseStream(ctx);
			loopCount++;
		}
        // usleep(1000); // Мінімальна пауза
	}
...
int main(int argc, char *argv[]) {
    ...
    // 2. Для H.265 профіль має бути Main (0)
    if (enCodecType == RK_CODEC_TYPE_H265) {
        ctx->venc.stChnAttr.stVencAttr.u32Profile = 0; // Main Profile
    }
